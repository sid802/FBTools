...
def insert_group(values_array, db_cursor):
    ...
    GROUP_INSERT = r"INSERT INTO GROUPS (ID, NAME_R, USERNAME, MEMBERS_AMOUNT, PRIVACY, " \
                   r"DESCRIPTION, CATEGORY, LAST_INFO_EXTRACTION) " \
                   r"VALUES (%(id)s, %(name)s, %(user)s, %(members)s, %(priv)s, %(desc)s," \
                   r"%(cat)s, %(last)s) " \
                   r"ON DUPLICATE KEY " \
                   r"UPDATE NAME_R=%(name)s, USERNAME=%(user)s, MEMBERS_AMOUNT=%(members)s, " \
                   r"PRIVACY=%(priv)s, DESCRIPTION=%(desc)s, CATEGORY=%(cat)s, LAST_INFO_EXTRACTION=%(last)s"
    ...
    db_cursor.execute(GROUP_INSERT,
                             {'id': group_id, 'name': group_name, 'user': group_user,
                              ...
                              'cat': category, 'last': datetime.now()
                              }
                             )
...
def import_file(file_path, delimiter='\t'):
    ...
    print 'DONE IMPORTING file: {0}'.format(file_path)
